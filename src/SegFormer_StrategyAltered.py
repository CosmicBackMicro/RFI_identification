...
class UNetLightningModule(pl.LightningModule):
    ...
    def sliding_window_inference(self, images: torch.Tensor, window_size: int = 640, stride: int = 640) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # Restore original training state afterwards to avoid side effects.
...
def get_stable_training_augmentation(size: int = 640):
    """
    点源与弱信号保护增强策略（保守版，针对射电时频图优化）：
    1. 使用 RandomCrop 代替 CenterCrop，增加位置泛化性。
    2. 引入极低比例的 ShiftScale，模拟仪器增益波动和频率漂移。
    3. 显著降低 Dropout 强度，保护稀疏的 point/pulsar 分布。
    4. 移除 RandomRotate90，避免混淆水平/垂直 RFI 的几何方向性。
    """
    train_transform = [
        # 基础几何变换（保持时频图的方向性，避免旋转以防混淆水平/垂直 RFI）
        albu.HorizontalFlip(p=0.5),
        albu.VerticalFlip(p=0.5),
        
        # 保守的空间变换：仅允许 5% 的位移和缩放，不旋转（防止点源形状畸变）
        ...
        # 减弱 Dropout：孔洞尺寸减小，避免覆盖掉唯一的 point 信号
        albu.CoarseDropout(
            num_holes_range=(1, 3), 
...
def get_validation_augmentation(height=640, width=640):
    """验证集保持确定性：仅进行必要的填充和中心裁剪以匹配训练尺寸"""
    test_transform = [
        albu.PadIfNeeded(min_height=height, min_width=width, border_mode=cv2.BORDER_CONSTANT),
        albu.CenterCrop(height=height, width=width, p=1.0)
    ]
...
if __name__ == "__main__":
    ...
    POINT_WEIGHT_MULTIPLIER = 6.0  # 小目标 point 进一步提高类别权重
    POINT_OVERSAMPLE_FACTOR = 3.0
    POINT_CROP_PROB = 0.6
    POINT_AUX_LOSS_WEIGHT = 0.25
    TRAIN_SAMPLES_PER_EPOCH = 4096  # 减少每轮采样数，加快 Epoch 迭代反馈
    ...
    # 恢复限制步数的 RandomSampler，以提升训练响应速度并处理超大数据集
    # 8192 样本 / batch_size 2 = 4096 步，如果是梯度累积 2，即 2048 步更新一次权重
    ...
    trainer.fit(
        ...
        ckpt_path="/home/cbm/deRFI/checkpoints/train/best_model-epoch=06-fgmIoU_val_fg_miou=0.5770.ckpt",
    )
