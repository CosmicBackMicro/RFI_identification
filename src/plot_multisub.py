...
def main():
    # 定义样本索引变量 (用于第一行模拟数据)
    block_idx = 2  # 控制读取哪个样本 (0, 1, 2, ...) - 改为0以匹配用户意图(尽管用户没明说，但block0最稳)

    # 路径定义
    SIM_PATH = "simulation_and_compare/simulation_psrfits.fits"
    # REAL_PATH 指向用户指定的 G30.00+6.44 真实数据块
    REAL_PATH = f"infer_and_compare/images/G30.00+6.44_20240120_block{block_idx}.fits"  
    
    # Row 1 (Sim) 的 Mask 路径
    sim_masks = {
        "AOFlagger": f"simulation_and_compare/mask_AOFlagger/simulation_psrfits_block{block_idx:04d}.png",
        "Model A": f"simulation_and_compare/mask_MiTUNet/simulation_psrfits_block{block_idx}.png",
        "Model B": f"simulation_and_compare/mask_SegFormer/simulation_psrfits_block{block_idx}.png",
        "GT": f"simulation_and_compare/mask_GroundTruth/simulation_psrfits_block{block_idx:04d}.png"
    }
    ...
    real_masks = {
        "AOFlagger": f"infer_and_compare/mask_AOFlagger/{raw_basename_full}{real_block_suffix}",
        "MiTUNet": f"infer_and_compare/mask_MiTUNet/{raw_basename_full}{real_block_suffix}",
        "SegFormer": f"infer_and_compare/mask_SegFormer/{raw_basename_full}{real_block_suffix}",
        "ReadFASTData": f"infer_and_compare/mask_ReadFASTData/{readfast_basename}{readfast_suffix}"
    }
