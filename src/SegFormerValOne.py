...
class UNetLightningModule(pl.LightningModule):
    ...
    @torch.no_grad()
    def sliding_window_inference(self, images: torch.Tensor, window_size: int = 640, stride: int = 640) -> tuple[torch.Tensor, torch.Tensor]:
        # Ensure the module is in eval() mode during sliding-window inference.
        # Restore original training state afterwards to avoid side effects if
        # this is invoked while the module is still in training mode.
        was_training = self.training
        self.eval()
        try:
            b, _, h, w = images.shape
            device = images.device
            logits_sum = torch.zeros((b, self.num_classes, h, w), device=device, dtype=torch.float32)
            count_map = torch.zeros((b, 1, h, w), device=device, dtype=torch.float32)

            y_positions = list(range(0, max(1, h - window_size + 1), stride))
            x_positions = list(range(0, max(1, w - window_size + 1), stride))
            if y_positions[-1] != max(0, h - window_size):
                y_positions.append(max(0, h - window_size))
            if x_positions[-1] != max(0, w - window_size):
                x_positions.append(max(0, w - window_size))

            for top in y_positions:
                for left in x_positions:
                    patch = images[:, :, top:top + window_size, left:left + window_size]
                    patch_h, patch_w = patch.shape[-2:]
                    patch_logits, _ = self(patch)
                    logits_sum[:, :, top:top + patch_h, left:left + patch_w] += patch_logits.float()
...
class FITSDataset(Dataset):
    ...
    def _point_focused_crop(self, image: np.ndarray, mask_labels: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        elif self.point_crop_prob == 0.0:
            # 💡 验证阶段逻辑：当 point_crop_prob=0 时，强制从中心裁剪窗口
            # 这样对于大于 crop_size 的验证图，评估的是其中心区域而非左上角
...
if __name__ == "__main__":
    ...
    FOCAL_GAMMA = 2.0
    LOSS_ALPHA = 0.4  # 降低对 FN 的权重，避免 point 召回过强导致误检上升
    LOSS_BETA = 0.6   # 提高对 FP 的惩罚，优先改善 point Precision / IoU
    ...
    POINT_WEIGHT_MULTIPLIER = 5.0  # 小目标 point 进一步提高类别权重
    ...
    augmentation=get_stable_training_augmentation(TRAIN_CROP_SIZE),
        ...
        pulsar_oversample_factor=2.0, # 采样倍率回调至 2.0
    ...
    # 给脉冲星类别适度增加权重，从 5.0 降至 1.5 以防过拟合
    pulsar_class_index = CLASSES.index("pulsar")
    class_weights[pulsar_class_index] *= 1.5
    print(f"[ClassWeights] point 类权重额外乘以 {POINT_WEIGHT_MULTIPLIER:.2f}，调整后={class_weights[point_class_index]:.4f}")
    print(f"[ClassWeights] pulsar 类权重额外乘以 1.50，调整后={class_weights[pulsar_class_index]:.4f}")
    ...
    trainer.validate(
        ...
        # ckpt_path="/home/cbm/deRFI/checkpoints/train/best_model-epoch=09-fgmIoU_val_fg_miou=0.6808.ckpt",
        ckpt_path="/home/cbm/deRFI/checkpoints/train/best_model-epoch=09-fgF1_val_fg_macro_f1=0.7595.ckpt"
    )
    
    print("=" * 60)
    print("🎉 验证完成！结果已保存至 results/metrics/")