...
# 手动指定要绘制的样本名（不带扩展名）。
# 例如：
# MANUAL_SAMPLE_BASENAMES = [
#     "simulation_psrfits_block0001",
#     "G150.00+0.00_20230220_block5085",
# ]
# 当该数组非空时，会优先按此顺序读取与排版（覆盖随机采样）。
MANUAL_SAMPLE_BASENAMES: List[str] = [
    # 1. Horizontal
    "G135.96-8.90_20220225_block2892",
    # 2. Point
    "G72.95-1.53_20251108_block1",
    # 3. Block
    "G200.14+2.80_20250409_block79",
    # 4. Vertical
    "G31.37+2.20_20201111_block1",
    # 5. Pulsar
    "B0355+54_20191110_downsamp8_block139"
]

SAMPLE_DISPLAY_NAMES: Dict[str, str] = {
    "G135.96-8.90_20220225_block2892": "Horizontal",
    "G72.95-1.53_20251108_block1": "Point",
    "G200.14+2.80_20250409_block79": "Block",
    "G31.37+2.20_20201111_block1": "Vertical",
    "B0355+54_20191110_downsamp8_block139": "Pulsar"
}
...
def plot_samples_grid(
    ...
    fig_h = max(4.0, rows * 2.8 + 1.2)  # Added 1.2 to the total height for the legend at the top
    ...
    for idx, (sample_name, fits_path, mask_path) in enumerate(selected):
        ...
        if idx == 0:
            ax_right.set_title("With Mask Overlay", fontsize=18, pad=10, fontweight='bold')
        ...
        if sample_name == "G200.14+2.80_20250409_block79":
            ...
            cbar = fig.colorbar(sm, ax=ax_right, pad=0.01, fraction=0.046)
        else:
            cbar = fig.colorbar(im_left, ax=ax_right, pad=0.01, fraction=0.046)
        # -------------------------------------------------------------------------
    ...
    fig.tight_layout(rect=(0, 0, 1, 0.94))  # Leaving space for the global legend at top
