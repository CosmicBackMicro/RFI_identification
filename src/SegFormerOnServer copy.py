...
class UNetLightningModule(pl.LightningModule):
    ...
    def on_validation_epoch_end(self):
        ...
        conf_mat_norm = conf_mat.astype('float') / (row_sums[:, np.newaxis] + 1e-9)

        # 3. 绘图
        plt.figure(figsize=(12, 10))
        sns.heatmap(conf_mat_norm, annot=True, fmt=".3f", cmap='Blues',
                    xticklabels=self.class_names, yticklabels=self.class_names)
        plt.xlabel('Predicted Label')
        plt.ylabel('True Label')
        plt.title(f'Confusion Matrix (Recall) - Epoch {self.current_epoch}')
        plt.tight_layout()

        # 4. 转换并上传到 TensorBoard / Logger
        buf = io.BytesIO()
        ...
        csv_path = f"results/metrics/last_epoch_metrics.csv"
        ...
        tbl.set_fontsize(12)
        tbl.scale(1.2, 1.2)
        plt.title('LAST EPOCH VALIDATION METRICS PER CLASS', fontsize=14, pad=16)
        pdf_path = "results/metrics/last_epoch_metrics.pdf"
        with PdfPages(pdf_path) as pdf:
            pdf.savefig(fig, bbox_inches='tight')
        plt.close(fig)
        print(f"Saved metrics table as PDF: {pdf_path}")
...
def get_preprocessing(preprocessing_fn):
    ...
    return albu.Compose(_transform, p=1.0)
...
if __name__ == "__main__":
    ...
    TRAIN_SAMPLES_PER_EPOCH = 8192  # 减少每轮采样数，加快 Epoch 迭代反馈，推荐 8192
